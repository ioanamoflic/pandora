    ...
    while run_nr > 0 loop
        for cx in
            (select * from (select * from linked_circuit lc tablesample system_rows(sys_range)) as it
                            ...
                            and visited = false for update skip locked)
        loop
            if cx.id is not null then
                ...
                select count(*) into distinct_existing from
                     (select * from linked_circuit where id in (cx_prev_q1_id, cx_prev_q2_id, cx_next_q1_id, cx_next_q2_id)
                                                   order by id for update skip locked) as it;

create or replace procedure cancel_two_qubit(type_1 int, type_2 int, parameter float, sys_range int, run_nr int)
    language plpgsql
as
$$
declare
...
begin
	while run_nr > 0 loop
    	...
    	select * into first from (select * from linked_circuit lc tablesample system_rows(sys_range)) as it
    	                    where it.type=type_1 and it.param=parameter for update skip locked limit 1;
    	if first.id is not null then
			...
			if second.id is not null and second.type=type_2 and second.param=parameter then
                ...
                if first.id is not null and first.type=type_2 and first.param=parameter then
                    first_prev_q1 := div(first.prev_q1, 10);
                    first_prev_q2 := div(first.prev_q2, 10);
                    second_next_q1 := div(second.next_q1, 10);
                    second_next_q2 := div(second.next_q2, 10);

create or replace procedure rewrite_toffoli_pattern_c()
    language plpgsql
as
$$
declare
    ...
    while found = true loop
        ...
        if toffoli.id is not null then
            ...
            if cnot.id is not null then
                ...
                -- Gate 1: cnot(2,3) - First gate connects to original Toffoli's previous gates
                ...
                -- Gate 3: TOFFOLI(0,1,2) - connects to gate2
                ...
                update linked_circuit set (next_q1, next_q2, next_q3) = (gate2_id * 10 + 0, gate3_id * 10 + 1, null) where id = gate1_id;

                -- Gate2 connections
                update linked_circuit set (prev_q1, prev_q2, prev_q3) = (toffoli.prev_q1, toffoli.prev_q2, toffoli.prev_q3) where id = gate2_id;
                update linked_circuit set (next_q1, next_q2, next_q3) = (gate3_id * 10 + 0, gate3_id * 10 + 0, cnot.next_q1) where id = gate2_id;

                -- Gate3 connections
                update linked_circuit set (prev_q1, prev_q2, prev_q3) = (gate2_id * 10 + 0, gate2_id * 10 + 0, gate1_id * 10 + 1) where id = gate3_id;
                update linked_circuit set (next_q1, next_q2, next_q3) = (toffoli.next_q1, toffoli.next_q2, cnot.next_q2) where id = gate3_id;

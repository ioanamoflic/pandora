create or replace procedure commute_cx_ctrl_target(sys_range integer, run_nr integer)
    language plpgsql
as
$$
declare
...
begin
	while run_nr > 0 loop
    	...
    	select * into first_cx from (select * from linked_circuit lc tablesample system_rows(sys_range)) as it

create or replace procedure linked_hhcxhh_to_cx_bernoulli(sys_range int, run_nr int)
    language plpgsql
as
$$
declare
...
begin
    ...
    while run_nr > 0 loop
        ...
        select * into cx from (select * from linked_circuit lc tablesample bernoulli(sys_range)) as it where type = 'CXPowGate' for update skip locked limit 1;
            ...
            if distinct_count = distinct_existing then
                if left_q1.type = 'HPowGate' and left_q2.type = 'HPowGate' and right_q1.type = 'HPowGate' and right_q2.type = 'HPowGate' then
                    ...
                    update linked_circuit set (switch, prev_q1, prev_q2, next_q1, next_q2, visited)
                                = (not cx.switch, left_q2.prev_q1, left_q1.prev_q1, right_q2.next_q1, right_q1.next_q1, false) where id = cx.id;

...
begin
	while run_nr > 0 loop
    	...
    	if cx.id is not null then
            ...
            select * into sg from linked_circuit_qubit where id = cx_next_id for update skip locked;
    	    ...
    	    if sg.id is not null and sg.type = single_type then
			    ...
			    select count(*) into distinct_existing from (select * from linked_circuit_qubit where id in (sg_next_id, cx_next_id) for update skip locked) as it;

    	        if distinct_count = distinct_existing then
    	            modulus_prev := 'next_q' || mod(cx.prev_q1, 10) + 1;
			        modulus_next := 'prev_q' || mod(sg.next_q1, 10) + 1;

                    execute 'update linked_circuit_qubit set ' || modulus_prev || ' = $1 where id = $2' using sg.id * 10, cx_prev_id;
			        execute 'update linked_circuit_qubit set ' || modulus_next || ' = $1 where id = $2' using cx.id * 10, sg_next_id;

                    update linked_circuit_qubit set (next_q1, prev_q1, visited) = (sg.next_q1, sg.id * 10, true) where id = cx.id;
                    update linked_circuit_qubit set (next_q1, prev_q1) = (cx.id * 10, cx_prev_q1) where id = sg.id;
                end if;
    	        run_nr = run_nr - 1;
			end if;
    	    commit;

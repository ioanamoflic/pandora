...
begin
	while run_nr > 0 loop
    	...
    	select * into first_cx from (select * from linked_circuit lc tablesample bernoulli(sys_range)) as it where it.type='CXPowGate' for update skip locked limit 1;
    	if first_cx.id is not null then
            ...
            if mod_first_prev_q2 = 0 then
                ...
                if second_cx.id is not null and second_cx.type = 'CXPowGate' then
                    ...
                    if distinct_count = distinct_existing then
                        ...
                        insert into linked_circuit values (default, first_cx.id * 10, second_cx.id * 10 + 1, null, 'CXPowGate', 1, false, first_cx.next_q1, second_cx.next_q2, null, false, first_cx.label, false, null)
